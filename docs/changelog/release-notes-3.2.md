...
- Added support for Mixbus 11, 12, and 13 on macOS.

#### Changed
- FaderPort 8/16 mode buttons switch between modes more consistently.
...
- FaderPort 8/16: fixed pan display for Routing Folder tracks, restoring the previous mode after an alert, inserts mode on FaderPort 16, and the track browser when banked before the first track.
- Fixed Pro Tools version detection on Windows.
- Fixed crashes when closing or switching setups and plug-in editors.

### [3.2.0.25] - 2026-02-23
Version 3.2.0.25 supports all surfaces. Earlier 3.2 pre-releases were for FaderPort 8/16 only.
...
- Added support for Premiere Pro 2023, 2024, and 2025 on macOS.
...
- Added `Open Troubleshooting Guide` to the menu on macOS and Windows, replacing `Show log file`. In this version it opens the log file instead of the troubleshooting guide. Fixed in 3.2.2.

#### Changed
- FaderPort 8/16 mode buttons were reworked. Navigation now defaults to `Bank`.
...
#### Fixed
- FaderPort 8: fixed switching between Pan, Sends, and Plug-in modes.
- FaderPort 8/16: track focus no longer presses an insert twice when changing banks.
...
- Pro Tools on Windows: modifier buttons now match Pro Tools, and fixed plug-in control.
